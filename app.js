...
//create a local login strategy that simply logs in the test user when passport.authenticate('local') is called 
//and then creates a session and sets cookies. used only in test mode
const testStrategy = new LocalStrategy(
  function (username, password, done) { //expects a username and password arg as usualluy used for local authentication..in this case we don't
    ...
    user.login = "testUser"
    user.id = testData["pull_request"]["user"]["id"]
    user.githubPicture = "#"
    user.email = "testemail"

    return databaseStore.storeUserDetailsAsync(user.id, user)
      .then(function () {
...
app.use(expressSession({
  cookie: { secure: "auto" }, //use auto ... if set to true server need to be running on https
  //when deployed on heroku we run as http behind an https proxy
  //so need to set trust proxy settings in express to trust 1st proxy...see furher up code
  //setting to true creates problems with testing as cookies arent send unless you send https requests to server
  //chai uses agent which is based on superagent to send requests that send cookie info however this will not 
  //work with self signed certs for https. 
  //so set this to auto and it will switch from true to fakse if it detects a http request is being sent.
  //deployed version will always be behind a https proxy anyway
